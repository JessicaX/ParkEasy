...
let file = require('/Users/jessica_x_1028/Desktop/hello/convertData/convertcsv.json');
//update json objects
function setCoordinate(jsonObj) {
    for (var i = 0; i < jsonObj.length; i++) {
        console.log( jsonObj[i].x_coord, jsonObj[i].y_coord )
        console.log( CnvEN2LL(jsonObj[i].x_coord, jsonObj[i].y_coord, 5) )  
        var temp = CnvEN2LL(jsonObj[i].x_coord, jsonObj[i].y_coord, 5).split(',')
        var x = Number( temp[0] )
        var y = Number( temp[1] )
    ...
    var fs = require("fs");
    fs.writeFile("./object.json", JSON.stringify(jsonObj), (err) => {
        if (err) {
            console.error(err);
            return;
        };
...
// print( CnvEN2LL(33758.4143,33695.5198,5) )